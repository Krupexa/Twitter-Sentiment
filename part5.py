...
fun = pd.read_csv('fun_dummy_split.csv')
happy = pd.read_csv('happy_dummy_split.csv')
sad = pd.read_csv('sad_dummy_split.csv')
upset = pd.read_csv('upset_dummy_split.csv')

#Get the wordbag
wordbag = pd.read_csv('wordbag_dummy.csv')
wordbag = wordbag.drop_duplicates()

#Classify tweets that had the word fun or happy in them as 1 (positive) and the others as 0 (negative)
fun['type'] = 1
happy['type'] = 1
upset['type'] = 0
sad['type'] = 0
...
# calculate probability of each word accurerancy
...
for i in range(len(frequency)):
    ...
    for j in range(len(train_positive)):
        ...
        # If the word appears at least once, we count it as that tweet having it. We sum over all the tweets that the word appears at least once in
        if appears > 0:
            count = count + 1
    positive[i] = count
...
d = {'word': word_bank, 'positive': positive, 'negative': negative, 'Word_prob': accu}

ftable = pd.DataFrame(data = d)
ftable.to_csv('ftable_dummy.csv')
print(positive_instance)
print(negative_instance)
